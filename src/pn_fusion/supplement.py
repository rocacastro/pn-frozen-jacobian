"""Build the reader-facing supplementary computational material."""
...
\hypersetup{hidelinks,pdftitle={Supplementary Computational Material: Frozen-Jacobian Methods with Predictor and Memory},pdfauthor={Rodrigo Castro Marin}}
\renewcommand{\thetable}{S\arabic{table}}
\renewcommand{\thefigure}{S\arabic{figure}}
\title{Supplementary Computational Material\\\large Frozen-Jacobian methods with predictor and memory: order, optimal selection, and fusion}
\author{Rodrigo Castro Mar\'in}
\date{}
\begin{document}\maketitle
This supplement provides detailed numerical breakdowns and secondary graphical representations supporting the computational experiments reported in the accompanying article. Full machine-readable datasets and the computational code are provided in the repository. All timing values reported below are archived measurements obtained under the experimental protocols described in the article.

\section{Detailed external-comparator timings}
Tables~\ref{tab:p5} and~\ref{tab:p7} report the detailed results at $\mathrm{TOL}=10^{-12}$. The article summarizes all nine configurations per field. Full-precision CSV files, including the other tolerances, are supplied in \texttt{data/reference/p5\_m6\_results.csv} and \texttt{p7\_m8\_results.csv}.
...
Table~\ref{tab:optima} reports all six control points. The article displays two representative endpoints. Stability throughout $[1,4.5]$ follows from affine normalized-cost differences and the unimodality argument, not from this finite list alone.
...
Figures~\ref{fig:external} and~\ref{fig:fusion} provide graphical views of the secondary comparisons. The underlying measurements are the same as those reported in the data files. A configuration range is not an IQR or confidence interval. No interpolation establishes a universal time crossover or monotonicity.
...
\end{figure}
\paragraph{Data provenance.} For $G_5$, the archived fusion work columns were computed with the evaluation-cost convention stored with the original runs; this is documented in \texttt{docs/KNOWN\_ISSUES.md}. The repository also provides work values recomputed with the canonical cost convention used in the article. The elapsed-time values in Figure~\ref{fig:fusion} are unaffected.
\end{document}
'''
